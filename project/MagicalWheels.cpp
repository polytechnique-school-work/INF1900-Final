...
uint16_t MagicalWheels::turn(Direction direction) {
    ...
    while (Clock::getTimestamp() < stopTime) {
        ...
        while (Clock::getTimestamp() < stopTime) {
            // Faire en sorte que si le fetch a une certaine valeur de retour, on skip le while (et
            // on effectue pas le change direction).
            uint16_t hasFindSomething = this->fetch(direction);
            if (hasFindSomething <= SECOND_DISTANCE) {
                DEBUG_PRINT(("Fin de la rotation (45) : trouvé."));
                DEBUG_PRINT((hasFindSomething));
        ...
        // Temps terminé, on regarde si y'a un poteau dans le coin ou pas.
    ...
    uint16_t MagicalWheels::fetch(Direction direction) {
        ...
        /*
            TODO
            On doit faire en sorte de vérifier si y'a une valeur en bas d'une certaine valeur à
        chaque fois. Soit qu'on définie une valeur par défaut et il se dirige vers un bloc en
        dessous d'une certaine valeur. Vérifier que le bloc est pas trop loin. Retourner la
        distance.
        */
