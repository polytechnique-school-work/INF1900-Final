...
void SVG::init(){
    ...
    {
        char message[] = "<text x=\"10\" y=\"4\" fill=\"blue\" class = \"small\"> section01 -- equipe 020304 -- Crazy Frog </text>\n";
        transmettreMessage(message);
    }

    {
        char message[] = "<rect x = \"10\" y = \"5\" width = \"96\" height = \"48\" stroke = \"black\" stroke-width = \".2\" fill = \"white\" />\n";
        transmettreMessage(message);
    }

    {
        char message[] = "<rect x = \"19\" y = \"45\" width = \"1\" height = \"1\" stroke = \"red\" stroke-width = \".2\" fill = \"red\"/>\n";
        transmettreMessage(message);
    }
}
...
void SVG::ecrireRectangles(){
    ...
    for(uint8_t i = 0; i < nbPoints; i++){
        if(points[i].getIndex() == 1)
            continue;
        char message[110];
        sprintf(message, "<rect x = \"%d\" y = \"%d\" width = \"1\" height = \"1\" stroke = \"black\" stroke-width = \".2\" fill = \"black\"/>\n", points[i].getX(), points[i].getY());
        transmettreMessage(message);
        _delay_ms(10);
    }
}
...
void SVG::makeCircles() {
    for(uint8_t i = 0; i < nVisites; i++){
        char message[100];
        sprintf(message, "<circle cx = \"%d\" cy = \"%d\" r = \"1.5\" stroke = \"black\" stroke-width = \".2\" fill = \"gray\" />\n", pointsVisites[i].getX(), pointsVisites[i].getY());
        transmettreMessage(message);
    }
}
...
void SVG::makePolygon() {
    ...
    {
        char message[] ="\" fill = \"rgba(0, 255, 0, 0.25)\" stroke = \"black\" stroke-width = \".2\" />\n";
        transmettreMessage(message);
    }

    {
        ...
        sprintf(message, "<text x=\"10\" y=\"56\" fill=\"blue\" class=\"small\" > Aire: %d pouces carres </text>", area);
        transmettreMessage(message);
    }
}
